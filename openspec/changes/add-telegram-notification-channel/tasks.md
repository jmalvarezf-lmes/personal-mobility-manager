...
- [x] 4.3 `infrastructure/telegram_link.py` — `generate_link_token(user_id: UUID) -> str` and `verify_link_token(token: str) -> UUID`, using `itsdangerous.URLSafeTimedSerializer(get_jwt_secret(), salt="telegram-link")` (mirror `presentation/api/csrf.py`'s exact structure — read that file first), with its own max-age constant (a sensible short window, e.g. 10 minutes, similar in spirit to `csrf.py`'s `_STATE_MAX_AGE`). `verify_link_token` raises `ValueError` on expired/tampered tokens, mirroring `csrf.verify_signed_state`'s error-handling style.
