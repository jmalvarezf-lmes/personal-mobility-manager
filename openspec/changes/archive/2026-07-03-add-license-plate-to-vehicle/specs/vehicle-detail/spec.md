...
- **THEN** the response is HTTP 200 with `vehicle_id`, `brand`, `display_name`, `vin`, `license_plate`, and a `config` object containing `username`, `locale` and `password: "●●●●●●●●"`
...
- **THEN** the response is HTTP 200 with `vehicle_id`, `brand`, `display_name`, `license_plate`, and a `config` object containing `location_token`
...
#### Scenario: Vehicle with plate returns value in detail
- **WHEN** a vehicle has a license plate stored
- **THEN** the `license_plate` field in the detail response is the stored string
