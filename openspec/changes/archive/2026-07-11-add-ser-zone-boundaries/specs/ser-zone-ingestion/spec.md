...
**Reason**: Single-source point CSV download is replaced by a two-source pipeline (SER band shapefile + callejero CSV) that produces zone-boundary records instead of point records.
**Migration**: `MADRID_SER_CALLES_URL` is replaced by `SER_ZONE_SHP_URL` and `MADRID_CALLEJERO_URL`.

### Requirement: Parse SER spot fields from 218228 CSV
**Reason**: 218228 CSV parsing (spot-level fields `calle`, `color`, `gis_x`, `gis_y`, `numero_plazas`) is replaced by shapefile band parsing plus a callejero join, producing zone-level fields instead.
**Migration**: See ADDED "Parse and join SER zone boundary sources" below.

### Requirement: Use UTM coordinates directly without centimetre conversion
**Reason**: This requirement described point coordinate handling for the retired 218228 CSV format. Coordinate handling for the new sources is covered by the ADDED requirements below (the shapefile is already in UTM metres; callejero DMS lat/lng is converted then reprojected to UTM for the spatial join).
**Migration**: N/A — new parsing logic replaces this entirely.
