...
- FROM: `### Requirement: SerTicketTriggerHandler notifies the owner when a ticket is required`
- TO: `### Requirement: SerTicketNotificationTriggerHandler notifies the owner when a ticket is required`
...
- **THEN** the owner's `ser_zone_ticket_required` preference is not consulted, and `VehicleLocationRepository.get_previous` and `FindContainingSerZone.execute` are not called
