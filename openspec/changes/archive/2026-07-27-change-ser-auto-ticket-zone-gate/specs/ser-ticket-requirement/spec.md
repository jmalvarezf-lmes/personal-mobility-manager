...
Otherwise, it SHALL call `ParkingTicketRepository.find_all_active_for_vehicle(vehicle_id, at)`, which returns every one of the vehicle's tickets still active at `at` (not just a single one) — a vehicle can legitimately hold more than one concurrently-active ticket, one per SER zone it has entered while a previous zone's ticket has not yet expired. If **any** ticket in that list satisfies either of the following, it SHALL return `False` immediately:
- the ticket's `city_code` and `zone_number` are both `None` (a legacy ticket created before these fields existed) — its true zone can't be recovered, so it is treated as covering any zone (fail-safe against creating a duplicate paid ticket); or
- the ticket's `(city_code, zone_number)` equals `(zone.city_code, zone.zone_number)` — the vehicle already has an active ticket for this exact zone.

If no ticket in the list satisfies either condition (including when the list is empty), it SHALL NOT return based on the active-ticket check and SHALL fall through to the ambient-label check below, exactly as if the vehicle had no active tickets at all.

In either case where it returns `False` on the active-ticket check, it does so without consulting the ambient-label repository, the label-exemption rule, the manual-exemption repository, or the zone rule.
...
#### Scenario: No ticket required when the vehicle's active ticket is for this same zone
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, and `ParkingTicketRepository.find_all_active_for_vehicle(vehicle_id, at)` returns a list containing a ticket whose `(city_code, zone_number)` equals `(zone.city_code, zone.zone_number)`
...
#### Scenario: No ticket required when the vehicle's active ticket predates zone tracking
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, and `ParkingTicketRepository.find_all_active_for_vehicle(vehicle_id, at)` returns a list containing a ticket whose `city_code` and `zone_number` are both `None`
- **THEN** it returns `False`, without consulting the ambient-label repository, the label-exemption rule, the manual-exemption repository, or the zone rule

#### Scenario: Ticket still required when the vehicle's only active ticket is for a different zone
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, and `ParkingTicketRepository.find_all_active_for_vehicle(vehicle_id, at)` returns a list whose only ticket's `(city_code, zone_number)` does not equal `(zone.city_code, zone.zone_number)` and is not `(None, None)`
- **THEN** the active-ticket check does not itself return a result, and the use case proceeds to evaluate the ambient-label and exemption checks exactly as if the vehicle had no active ticket

#### Scenario: No ticket required when the vehicle holds two concurrently-active tickets for different zones and one matches
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, and `ParkingTicketRepository.find_all_active_for_vehicle(vehicle_id, at)` returns two active tickets — one for `zone`'s own `(city_code, zone_number)` and one for a different zone with a later `end_date`
- **THEN** it returns `False`, because the matching ticket is found regardless of which ticket has the latest `end_date` — this is the scenario the original single-ticket ("most recent by end_date") design got wrong, corrected after a 4R review

#### Scenario: No ticket required when the vehicle's ambient label is confirmed electric and exempt in that city
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, the vehicle has no active ticket (or only one for a different zone), `VehicleAmbientLabelRepository.get_by_vehicle_id(vehicle_id)` returns a row with `status == AmbientLabelStatus.FOUND` and `label == AmbientLabel.ZERO`, and the injected `SerLabelExemptionRule` dependency's `is_label_exempt(zone.city_code, label)` returns `True`
...
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, the vehicle has no active ticket (or only one for a different zone), and `VehicleAmbientLabelRepository.get_by_vehicle_id(vehicle_id)` returns `None`, or a row whose `status` is `AmbientLabelStatus.NOT_FOUND` or `AmbientLabelStatus.ERROR`
...
- **WHEN** `DetermineSerTicketRequirement.execute(zone, vehicle_id, at)` is called with a non-`None` `SerZone`, enforcement is active, the vehicle has no active ticket (or only one for a different zone), no label exemption applies, and the vehicle's stored exemption's `(city_code, zone_number)` does not equal `(zone.city_code, zone.zone_number)`
- **THEN** it returns `True`, without consulting the injected `SerExemptionZoneRule` dependency
...
- **THEN** the result depends only on zone presence, the injected enforcement-schedule dependency's answer, active-ticket state (now including its zone), the vehicle's ambient label and the injected `SerLabelExemptionRule` dependency's answer, the vehicle's stored exemption, and the injected `SerExemptionZoneRule` dependency's answer, since home-proximity is not wired in during this change
