...
- **THEN** all `vehicle_configs`, `vehicle_locations`, `vehicle_shares`, and `vehicle_ser_parking_exemptions` rows for that vehicle_id are gone (cascade)

#### Scenario: Sharee delete rejected
- **WHEN** an authenticated user sends `DELETE /vehicles/{id}` for a vehicle shared with them but not owned by them
...
- **THEN** `GET /vehicles/{id}` for the same id returns HTTP 404 for both the former owner and any former sharee
