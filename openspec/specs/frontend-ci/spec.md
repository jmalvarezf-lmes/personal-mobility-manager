...
The CI workflow SHALL include a `lint-frontend` job that runs on `ubuntu-latest`, installs pnpm via `pnpm/action-setup`, installs frontend dependencies with `pnpm install --frozen-lockfile` from the `frontend/` working directory, and then runs `pnpm type-check` and `pnpm lint` sequentially. The job SHALL fail if either command exits with a non-zero code.
